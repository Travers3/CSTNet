...
parser.add_argument('--data', type=str, default='data/Oil', help='data path')
parser.add_argument('--adjdata', type=str, default='data/Oil/adj_mat.pkl', help='adj data path')
...
parser.add_argument('--num_nodes', type=int, default=330, help='number of nodes')
...
parser.add_argument('--save', type=str, default='./garage/Oil', help='save path')
parser.add_argument('--expid', type=int, default=1, help='experiment id')
parser.add_argument('--model', type=str, default='OCST_net_tcn', help='adj type')
parser.add_argument('--decay', type=float, default=0.92, help='decay rate of learning rate ')
parser.add_argument('--days', type=int, default=24, help='time division of a day')
...
def main():
    ...
    dataloader = util.load_dataset(args.data, args.batch_size, args.batch_size, args.batch_size,days=args.days,
                                   sequence=args.seq_length, in_seq=args.in_len)
    scaler = dataloader['scaler']
    supports = [torch.tensor(i).to(device) for i in adj_mx]

    print(args)
    if args.model == 'OCST_net':
        engine = trainer(scaler, args.in_dim, args.seq_length, args.num_nodes, args.nhid, args.dropout,
                         args.learning_rate, args.weight_decay, device, supports, args.days, args.dims, args.order)
    elif args.model == 'OCST_net_tcn':
        engine = trainer9(scaler, args.in_dim, args.seq_length, args.num_nodes, args.nhid, args.dropout,
                          args.learning_rate, args.weight_decay, device, supports, args.decay
                          )

    # check parameters file
    params_path = args.save + "/" + args.model  # ./garage/XiAn_City/gwnet
    ...
    for iter, (x, y, ind) in enumerate(dataloader['test_loader'].get_iterator()):
        ...
        testx = testx.transpose(1, 3)
        with torch.no_grad():
            preds, spatial_at, parameter_adj = engine.model(testx, ind)
    ...
    for i in range(12):
        #pred = prediction[:, :, i]
        pred = scaler.inverse_transform(prediction[:, :, i])
...
def tes_model():
    ...
    if args.model == 'OCST_net':
        ...
        engine = trainer9(scaler, args.in_dim, args.seq_length, args.num_nodes, args.nhid, args.dropout,
                          args.learning_rate, args.weight_decay, device, supports, args.days, args.dims)

    print("start testing...", flush=True)

    engine.model.load_state_dict(torch.load("./garage/Oil/Sz/60-7500-70/OCST_net_tcn/OCST_net_tcn_exp1_best_3.58.pth"))
    engine.model.eval()
