...
#data字典
def load_dataset(dataset_dir, batch_size, valid_batch_size=None, test_batch_size=None,days=24, sequence=12,
                 in_seq=12):
    #scalar
    data = {}
    for category in ['train', 'val', 'test']:
        cat_data = np.load(os.path.join(dataset_dir, category + '.npz'))
        data['x_' + category] = cat_data['x'][:, -in_seq:, :, 0:2]
...
def load_dataset_(dataset_dir, batch_size, valid_batch_size= None, test_batch_size=None):
    data = {}
    for category in ['train', 'val', 'test']:
        cat_data = np.load(os.path.join(dataset_dir, category + '.npz'))
    ...
    data['train_loader'] = DataLoader_1(data['x_train'], data['y_train'], batch_size)
    data['val_loader'] = DataLoader_1(data['x_val'], data['y_val'], valid_batch_size)
    data['test_loader'] = DataLoader_1(data['x_test'], data['y_test'], test_batch_size)
    #data['scaler'] = scaler
    return data
